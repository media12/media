...
client.on("guildMemberAdd", member => {
        if(member.guild.id === "610271105785004052") {  // ايدي السيرفر
  const channel = member.guild.channels.find('id', '634532723922305044'); //ايدي الروم
if (!channel) return;
channel.send(`**<@${member.user.id}> Welcome To __Youtube Test__  ** ❤️ `)  
}});
client.on("guildMemberRemove", member => {
        if(member.guild.id === "513019757746847754") { 
  const channel = member.guild.channels.find('id', '634532723922305044'); 
if (!channel) return;
...
client.login('3qalu-KzKAlElYbyd251luUzVfld-vQG');
